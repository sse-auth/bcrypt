...
var accum = (function () {
  var pool: any, // randomness pool
    time: any, // start timestamp
    last: any; // last step timestamp
  ...
  var to: string | number | NodeJS.Timeout | null | undefined = null;
  ...
  function start() {
    ...
    if (typeof window !== "undefined" && window && window.addEventListener)
      window.addEventListener("load", loadCollector, false),
        ...
        window.addEventListener("touchmove", touchCollector as any, false);
    else if (
      typeof document !== "undefined" &&
      document &&
      (document as any).attachEvent
    )
      (document as any).attachEvent("onload", loadCollector),
        (document as any).attachEvent("onmousemove", mouseCollector);
  }
  ...
  function stop() {
    ...
    if (typeof window !== "undefined" && window && window.removeEventListener)
      window.removeEventListener("load", loadCollector, false),
        ...
        window.removeEventListener("touchmove", touchCollector as any, false);
    else if (
      typeof document !== "undefined" &&
      document &&
      (document as any).detachEvent
    )
      (document as any).detachEvent("onload", loadCollector),
        (document as any).detachEvent("onmousemove", mouseCollector);
  }
  ...
  function touchCollector(ev: { touches: any[]; changedTouches: any[] }) {
    if (!to || pool.length >= 255) return;
    try {
